...
object StickerPicker {
    fun show(activity: Activity,onPick: (String)->Unit,onImport: ()->Unit) {
        ...
        val dialog=AlertDialog.Builder(activity).setTitle("免費表情貼圖").setView(column).setNegativeButton("取消",null).setNeutralButton("匯入自己的貼圖") { _,_-> onImport() }.create()
        ...
        for((i,file) in files.withIndex()) {
            ...
            val image=ImageButton(activity).apply {
                ...
                background=Ui.rounded(0xfff2ebfa.toInt(),16f); contentDescription="表情貼圖 ${i+1}"
                setPadding(4,4,4,4)
