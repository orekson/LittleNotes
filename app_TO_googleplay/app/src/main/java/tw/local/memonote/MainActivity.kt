...
class MainActivity: Activity() {
    ...
    private fun showNotes() {
        ...
        if(notes.isEmpty()) {
            ...
            empty.addView(Ui.label(this,"寫下今天的小事，插入喜歡的表情貼圖，再放到桌面隨時看見。",15f,Ui.muted)); list.addView(empty)
        }
        ...
        list.addView(Ui.button(this,"貼圖來源與使用說明") {
            AlertDialog.Builder(this).setTitle("貼圖來源與使用說明").setMessage("桌面長按空白處 → 小工具 → 小小筆記，選一篇筆記後長按拉大。\n\n文字工具：選取文字套用顏色、彩虹或柔光；未選取時套用全文。背景淡化只影響背景。\n\n內建 31 張表情貼圖來自 Twemoji v14.0.3，圖像採 Creative Commons Attribution 4.0 International 授權，可商用但需標示來源。詳見專案內的 ASSET_SOURCES.md 與 Twemoji 授權檔。")
                .setPositiveButton("知道了",null).setNeutralButton("查看素材授權") { _,_-> startActivity(Intent(Intent.ACTION_VIEW,android.net.Uri.parse("https://github.com/twitter/twemoji"))) }.show()
        })
    }
