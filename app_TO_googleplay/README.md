...
Android 個人筆記 App 與可調整大小的桌面小工具。介面為繁體中文，資料儲存在手機本機，不需登入或連網。

## 安裝與使用

1. 將交付的 `LittleNotes-1.2.apk` 傳到 Android 手機，開啟安裝。已安裝舊版時直接覆蓋更新。若手機要求，為你用來開啟 APK 的檔案管理器允許「安裝未知應用程式」。
2. 開啟「小小筆記」，按「寫一篇新筆記」。
3. 輸入標題與內容。選取文字後套用色彩、彩虹、柔光；沒有選取時會套用整篇。使用「清除樣式」復原一般文字。
4. 本文下方的文字工具列可左右滑動，下一排是「貼圖」與「桌面預覽」。貼圖選單包含 31 張免費表情貼圖，也能匯入自己的貼圖。
...
- 每個小工具可獨立選不同的筆記。同篇筆記的文字、貼圖、背景修改會更新到相關小工具。
- 支援 Android 8.0 / API 26 以上；本次已驗證的系統及未驗證範圍見 `docs/verification.md`。
- 小工具的最大尺寸受桌面程式格線限制，無法覆蓋狀態列、系統導覽區或桌面保留區。
- 彩虹和柔光為靜態效果，不持續閃爍；內建 Twemoji 貼圖為 72px 靜態圖，放大後可能看見像素。可匯入較高解析度的個人貼圖。
...
- 內建貼圖來源與授權標示見 `ASSET_SOURCES.md`；本 Play 版本使用的 Twemoji 圖像採 CC BY 4.0，並隨附授權檔與來源說明。
